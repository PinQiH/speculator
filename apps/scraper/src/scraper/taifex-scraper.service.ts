...
@Injectable()
export class TaifexScraperService implements OnApplicationBootstrap {
    ...
    async fetchInstInvestorsTxoTrades(date: string) {
        ...
        return {
            ...
            sitcPutsNetOiVolume, //投信-賣權未平倉口數買賣淨額
            sitcPutsNetOiValue, //投信-賣權未平倉契約金額買賣淨額(千元)
            dealersCallsLongTradeVolume, //自營商-賣權買方交易口數
            dealersCallsLongTradeValue, //自營商-賣權買方交易契約金額(千元)
            dealersCallsShortTradeVolume, //自營商-賣權賣方交易口數
            dealersCallsShortTradeValue, //自營商-賣權賣方交易契約金額(千元)
            dealersCallsNetTradeVolume, //自營商-賣權交易口數買賣淨額
            dealersCallsNetTradeValue, //自營商-賣權交易契約金額買賣淨額(千元)
            dealersCallsLongOiVolume, //自營商-賣權買方未平倉口數
            dealersCallsLongOiValue, //自營商-賣權買方未平倉契約金額(千元)
            dealersCallsShortOiVolume, //自營商-賣權賣方未平倉口數
            dealersCallsShortOiValue, //自營商-賣權賣方未平倉契約金額(千元)
            dealersCallsNetOiVolume, //自營商-賣權未平倉口數買賣淨額
            dealersCallsNetOiValue, //自營商-賣權未平倉契約金額買賣淨額(千元)
            dealersPutsLongTradeVolume, //自營商-買權買方交易口數
            dealersPutsLongTradeValue, //自營商-買權買方交易契約金額(千元)
            dealersPutsShortTradeVolume, //自營商-買權賣方交易口數
            dealersPutsShortTradeValue, //自營商-買權賣方交易契約金額(千元)
            dealersPutsNetTradeVolume, //自營商-買權交易口數買賣淨額
            dealersPutsNetTradeValue, //自營商-買權交易契約金額買賣淨額(千元)
            dealersPutsLongOiVolume, //自營商-買權買方未平倉口數
            dealersPutsLongOiValue, //自營商-買權買方未平倉契約金額(千元)
            dealersPutsShortOiVolume, //自營商-買權賣方未平倉口數
            dealersPutsShortOiValue, //自營商-買權賣方未平倉契約金額(千元)
            dealersPutsNetOiVolume, //自營商-買權未平倉口數買賣淨額
            dealersPutsNetOiValue, //自營商-買權未平倉契約金額買賣淨額(千元)
        };
    }

    //取得臺指選擇權 Put/Call Ratio
    ...
    async fetchLargeTradersTxoPosition(date: string) {
        ...
        return {
            ...
            top5NonSpecificTxoPutBackMonthsShortOi, //五大非特定法人-賣權遠月契約賣方部位
    ...
    //取得小型臺指所有契約未沖銷量
    async fetchMxfMarketOi(date: string) {
        // 將 `date` 轉換成 `yyyy/MM/dd` 格式
        const queryDate = DateTime.fromISO(date).toFormat('yyyy/MM/dd');
        ...
        return { date, mxfMarketOi }; //小型臺指所有契約未沖銷量
    }

    //取得三大法人小型臺指多空未平倉量
    async fetchInstInvestorsMxfOi(date: string) {
        // 將 `date` 轉換成 `yyyy/MM/dd` 格式
        const queryDate = DateTime.fromISO(date).toFormat('yyyy/MM/dd');
    ...
    async fetchRetailMxfPosition(date: string) {
        ...
        return {
            date,
            retailMxfLongOi, //散戶小型臺指多方未平倉口數
            retailMxfShortOi, //散戶小型臺指空方未平倉口數
            retailMxfNetOi, //散戶小型臺指淨未平倉口數
    ...
    async onApplicationBootstrap() {
        // const instInvestorsTxfTrades = await this.fetchInstInvestorsTxfTrades('2023-09-07');
        // console.log(instInvestorsTxfTrades); //顯示三大法人臺股期貨交易資訊

        // const instInvestorsTxoTrades = await this.fetchInstInvestorsTxoTrades('2023-09-07');
        // console.log(instInvestorsTxoTrades); //顯示三大法人臺指選擇權交易資訊

        // const txoPutCallRatio = await this.fetchTxoPutCallRatio('2023-09-07');
        // console.log(txoPutCallRatio); //顯示臺指選擇權 Put/Call Ratio

        // const largeTradersTxfPosition = await this.fetchLargeTradersTxfPosition('2023-09-07');
        // console.log(largeTradersTxfPosition); //顯示大額交易人臺股期貨未沖銷部位

        // const largeTradersTxoPosition = await this.fetchLargeTradersTxoPosition('2023-09-07');
        // console.log(largeTradersTxoPosition); //顯示大額交易人臺股選擇權未沖銷部位

        // const mxfMarketOi = await this.fetchMxfMarketOi('2023-09-07');
        // console.log(mxfMarketOi); //顯示小型臺指所有契約未沖銷量

        // const instInvestorsMxfOi = await this.fetchInstInvestorsMxfOi('2023-09-07');
        // console.log(instInvestorsMxfOi); //顯示小型臺指所有契約未沖銷量

        // const retailMxfPosition = await this.fetchRetailMxfPosition('2023-09-07');
        // console.log(retailMxfPosition); //顯示散戶小台淨部位以及散戶小台多空比

        // const exchangeRates = await this.fetchExchangeRates('2023-09-07');
        // console.log(exchangeRates); //顯示外幣參考匯率
    }
}
